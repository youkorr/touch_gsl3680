...
void GSL3680::setup() {
  ...
  ESP_LOGI(TAG, "I2C pins: SDA=%d, SCL=%d, Frequency=%d Hz", 
  ...
  bus_config.flags.enable_internal_pullup = false;  // Utiliser les pull-ups externes
  
  esp_err_t ret = i2c_new_master_bus(&bus_config, &this->i2c_master_bus_handle_);
  if (ret != ESP_OK) {
  ...
  dev_config.scl_speed_hz = this->i2c_frequency_;
  
  ret = i2c_master_bus_add_device(this->i2c_master_bus_handle_, 
                                  &dev_config,
                                  &this->i2c_master_dev_handle_);
  if (ret != ESP_OK) {
  ...
  if (!communication_ok) {
    ...
    ESP_LOGE(TAG, "Check connections: SDA=GPIO%d, SCL=GPIO%d, Address=0x%02X", 
             this->sda_pin_, this->scl_pin_, this->i2c_address_);
    this->mark_failed();
    return;
  }
